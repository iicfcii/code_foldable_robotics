...
def not_removable_up(laminate,is_adhesive):
   ...
   :rtype: Laminate
    """
    return not_removable(laminate,True,is_adhesive)
...
def not_removable_down(laminate,is_adhesive):
   ...
   :rtype: Laminate
    """
    return not_removable(laminate,False,is_adhesive)
...
def not_removable(laminate,up,is_adhesive):
    ...
    not_removable_material = [laminate[start]]
    for i in range(start+step,end,step):
        not_removable_material.append(laminate[i] | not_removable_material[-1])
    not_removable_material = Laminate(*not_removable_material)
    not_removable_material = not_removable_material[::step]


    for i in range(start,end-step,step):
        # Adhesive above/below other material cannot be used as web
        if is_adhesive[i]:
            not_removable_material[i] |= not_removable_material[i+step]
        # Material above/below adhesive cannot be used as web
        if is_adhesive[i+step]:
            not_removable_material[i] |= not_removable_material[i+step]

    return not_removable_material
